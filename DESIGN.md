...
Technical reference for 看見數字背後 · 台灣性侵害統計. For setup and contribution instructions, see [English](README.md) or [繁體中文](README-ch.md).

## Architecture

The site replaces the original Svelte/Snowpack application. It uses TanStack Start, React 19, Vite+, Tailwind CSS 4, shadcn/ui, Motion, and Chart.js. The website and API run in the same application, using the verified dataset checked into the repository.
